...
def start_ss(keyfile, host_string, dlab_conf_dir, web_path, os_user, mongo_passwd, keystore_passwd, cloud_provider,
             service_base_name, tag_resource_id, account_id, billing_bucket, aws_job_enabled, dlab_path, billing_enabled,
             authentication_file, offer_number, currency, locale, region_info, ldap_login, tenant_id, application_id,
             hostname, data_lake_name, subscription_id, validate_permission_scope, dlab_id, usage_date,
             product, usage_type, usage, cost, resource_id, tags, report_path=''):
    try:
        if not exists(os.environ['ssn_dlab_path'] + 'tmp/ss_started'):
            ...
            if billing_enabled:
                ...
                params = '--cloud_provider {} --infrastructure_tag {} --tag_resource_id {} --account_id {} \
                    --billing_bucket {} --aws_job_enabled {} --report_path "{}" --mongo_password {} --dlab_dir {} \
                         --authentication_file "{}" --offer_number {} --currency {} --locale {} --region_info {} \
                         --dlab_id {} --usage_date {} --product {} --usage_type {} --usage {} \
                         --cost {} --resource_id {} --tags {}'.\
                            format(cloud_provider, service_base_name, tag_resource_id, account_id, billing_bucket, aws_job_enabled, report_path,
                                    mongo_passwd, dlab_path, authentication_file, offer_number, currency, locale, region_info, dlab_id,
                                    usage_date, product, usage_type, usage, cost, resource_id, tags)
