...
def ensure_matplot(os_user):
    if not exists('/home/' + os_user + '/.ensure_dir/matplot_ensured'):
        try:
            ...
            sudo('pip3 install matplotlib==1.5.3 --no-cache-dir')
...
def install_caffe2(os_user, caffe2_version):
    if not exists('/home/{}/.ensure_dir/caffe2_ensured'.format(os_user)):
        ...
        sudo('pip2 install flask graphviz hypothesis jupyter matplotlib==1.5.3 pydot python-nvd3 pyyaml requests scikit-image '
             'scipy setuptools tornado --no-cache-dir')
        sudo('pip3 install flask graphviz hypothesis jupyter matplotlib==1.5.3 pydot python-nvd3 pyyaml requests scikit-image '
             'scipy setuptools tornado --no-cache-dir')
