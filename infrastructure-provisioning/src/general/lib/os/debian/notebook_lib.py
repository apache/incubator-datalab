    ...
    if not exists('/home/' + os_user + '/.ensure_dir/additional_python_libs_ensured'):
        try:
            ...
            if os.environ['application'] in ('jupyter', 'zeppelin'):
                sudo('pip2 install NumPy=={} SciPy pandas Sympy Pillow sklearn --no-cache-dir'.format(
                    os.environ['notebook_numpy_version']))
                sudo('pip3 install NumPy=={} SciPy pandas Sympy Pillow sklearn --no-cache-dir'.format(
                    os.environ['notebook_numpy_version']))
            if os.environ['application'] in ('tensor', 'deeplearning'):
...
def install_cntk(os_user, cntk_version):
    if not exists('/home/{}/.ensure_dir/cntk_ensured'.format(os_user)):
        if os.environ['local_repository_enabled'] == 'True':
            ...
            sudo('pip3 install https://cntk.ai/PythonWheel/GPU/cntk-{}-cp35-cp35m-linux_x86_64.whl '
                 '--no-cache-dir'.format(cntk_version))
