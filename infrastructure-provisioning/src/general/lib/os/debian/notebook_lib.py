...
def ensure_r(os_user, r_libs):
    if not exists(datalab.fab.conn,'/home/' + os_user + '/.ensure_dir/r_ensured'):
        try:
            ...
            datalab.fab.conn.sudo("curl -sSL 'http://keyserver.ubuntu.com/pks/lookup?op=get&search=0xE298A3A825C0D65DFD57CBB651716619E084DAB9' | apt-key add -")
           #datalab.fab.conn.sudo('''bash -c -l 'apt-key adv --keyserver-options http-proxy="$http_proxy" --keyserver keyserver.ubuntu.com --recv-keys E298A3A825C0D65DFD57CBB651716619E084DAB9' ''')
