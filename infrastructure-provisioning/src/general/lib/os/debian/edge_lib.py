    ...
    try:
        if not os.path.exists('/tmp/nginx_installed'):
            ...
            with cd('/tmp/src/luarocks-2.2.2/'):
                ...
                sudo('make build')
                sudo('make install')
                sudo('wget https://luarocks.org/manifests/cdbattags/lua-resty-jwt-0.2.0-0.rockspec')
                sudo('luarocks install lua-resty-jwt-0.2.0-0.rockspec')
                sudo('wget https://luarocks.org/manifests/bungle/lua-resty-session-2.26-1.rockspec')
                sudo('luarocks install lua-resty-session-2.26-1.rockspec')
                sudo('wget https://luarocks.org/manifests/pintsized/lua-resty-http-0.15-0.rockspec')
                sudo('luarocks install lua-resty-http-0.15-0.rockspec')
                sudo('wget https://luarocks.org/manifests/hanszandbelt/lua-resty-openidc-1.7.2-1.rockspec')
                sudo('luarocks install lua-resty-openidc-1.7.2-1.rockspec')
                sudo('wget https://luarocks.org/manifests/starius/luacrypto-0.3.2-2.rockspec')
                sudo('luarocks install luacrypto-0.3.2-2.rockspec')
                sudo('luarocks install lua-cjson')
                sudo('wget https://luarocks.org/manifests/avlubimov/lua-resty-core-0.1.17-4.rockspec')
                sudo('luarocks install lua-resty-core-0.1.17-4.rockspec')
