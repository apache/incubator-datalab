...
def ensure_jupyter_docker_files(os_user, legion_dir, templates_dir, jupyter_dir, spark_script, jupyter_conf_file, jupyter_version, docker_jupyter_conf, exploratory_name):
    if not exists('/home/' + os_user + '/.ensure_dir/jupyter_ensured'):
        try:
            run('git clone https://github.com/legion-platform/legion.git')
            with cd('/home/' + os_user + '/legion'):
                run('git checkout feat/887-jupyterlab-locally')
            sudo('rm /home/' + os_user + '/legion/containers/toolchains/python/Dockerfile')
...
#           sudo('sed -i \'s/3.5/3.6/g\' {}py3spark_local_template.json'.format(jupyter_dir))
           sudo('mv /tmp/jupyter_run.sh /home/' + os_user + '/legion/containers/toolchains/python/jupyter_run.sh')
           sudo('mv /tmp/Dockerfile_jupyter /home/' + os_user + '/legion/containers/toolchains/python/Dockerfile')
           sudo('sed -i \'s/nb_user/{}/g\' /home/' + os_user + '/legion/containers/toolchains/python/Dockerfile'.format(os_user))
...
#           sudo('sed -i \'s/scala_version/{}/g\' {}Dockerfile_jupyter'.format(os.environ['notebook_scala_version'], jupyter_dir))
           sudo('sed -i \'s/CONF_PATH/{}/g\' /home/' + os_user + '/legion/containers/toolchains/python/jupyter_run.sh'.format(docker_jupyter_conf))
