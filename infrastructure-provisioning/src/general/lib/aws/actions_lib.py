        ...
        if region == 'cn-north-1':
            try:
                ...
                local(venv_command + ' && sudo -i ' + pip_command +
                      ...
                      format(pip_mirror))
                local(venv_command + ' && sudo -i ' + pip_command +
                      ' install -i https://{0}/simple --trusted-host {0} --timeout 60000 boto boto3 NumPy SciPy Matplotlib==1.5.3 pandas Sympy Pillow sklearn --no-cache-dir'.
                      format(pip_mirror))
            ...
            local(venv_command + ' && sudo -i ' + pip_command +
                  ' install boto boto3 NumPy SciPy Matplotlib==1.5.3 pandas Sympy Pillow sklearn --no-cache-dir')
            # Need to refactor when we add GPU cluster
            if application == 'deeplearning':
                local(venv_command + ' && sudo -i ' + pip_command +
