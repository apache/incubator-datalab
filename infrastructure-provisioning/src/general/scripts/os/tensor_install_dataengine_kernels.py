...
def configure_notebook(keyfile, hoststring):
    ...
    run('mkdir -p /tmp/{}/'.format(args.tensor_dataengine_create_configs))
    if not exists('/tmp/tensor_dataengine_create_configs.py'):
        put(scripts_dir + 'tensor_dataengine_create_configs.py',
            '/tmp/tensor_dataengine_create_configs.py')
