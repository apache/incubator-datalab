...
def configure_zeppelin(os_user):
    if not exists(conn,'/home/' + os_user + '/.ensure_dir/zeppelin_ensured'):
        try:
            # test nexus
            conn.sudo('wget https://nexus.develop.dlabanalytics.com/repository/packages-public/zeppelin-0.9.1-prebuilt.tar.gz -O /tmp/zeppelin-0.9.1-prebuilt.tar.gz')
            conn.sudo('tar -zxvf /tmp/zeppelin-0.9.1-prebuilt.tar.gz -C /opt/')
            conn.sudo('ln -s /opt/zeppelin-0.9.1-prebuilt.tar.gz /opt/zeppelin')
