...
public class ProcessInfoBuilder implements Supplier<ProcessInfo>, Testing, TimeoutAction, Expireable {
	...
	public static void failed(ProcessInfoBuilder b) {
		b.status = FAILED;
		b.setReady();
	}

	public static void stop(ProcessInfoBuilder b) {
		if (b.p != null) {
			b.p.destroy();
		}
	...
	public static void kill(ProcessInfoBuilder b) {
		if (b.p != null) {
			b.p.destroyForcibly();
		}
		if (b.status != LAUNCHING && b.status != RUNNING) {
		...
		DlabProcess.getInstance().getUsersExecutorService(processId.getUser()).submit(() -> {
			...
			DlabProcess.getInstance().getExecutorService().execute(() -> {
				try {
					...
					DlabProcess.getInstance().getExecutorService().execute(() -> readStdOutStream(stdOutStream));
					InputStream stdErrStream = p.getErrorStream();
					DlabProcess.getInstance().getExecutorService().execute(() -> readStdErrorStream(stdErrStream));
					status = RUNNING;
					int exit = p.waitFor();
			...
			try {
				...
				LOG.warn("Exception occurred while getting result: {}", e.getMessage());

			}
		});
	}

	private void readStdErrorStream(InputStream stdErrStream) {
	...
	private void readStdOutStream(InputStream stdOutStream) {
