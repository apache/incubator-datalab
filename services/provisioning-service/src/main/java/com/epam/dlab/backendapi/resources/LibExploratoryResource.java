...
@Slf4j
public class LibExploratoryResource implements DockerCommands {
    ...
    public String getLibList(@Auth UserInfo ui, ExploratoryActionDTO<?> dto) throws IOException, InterruptedException {
        return action(ui.getName(), dto, DockerAction.LIB_LIST);
    }
    ...
    private FileHandlerCallback getFileHandlerCallback(DockerAction action, String uuid, ExploratoryBaseDTO<?> dto) {
        switch (action) {
            ...
            case LIB_LIST:
                return new LibListCallbackHandler(selfService, DockerAction.LIB_LIST, uuid, dto.getAwsIamUser(), dto.getNotebookImage());
        }
