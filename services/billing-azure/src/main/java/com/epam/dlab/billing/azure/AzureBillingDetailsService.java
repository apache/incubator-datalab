    ...
    private void updateBillingDetails(String user, List<Document> billingDetails) {
        if (!billingDetails.isEmpty()) {
            ...
            billingDetails.stream()
                    .filter(e -> DlabResourceType.EXPLORATORY.toString().equals(e.getString(MongoKeyWords.RESOURCE_TYPE)))
                    .forEach(aggregator);

            billingDetails.stream()
                    .filter(e -> DlabResourceType.COMPUTATIONAL.toString().equals(e.getString(MongoKeyWords.RESOURCE_TYPE)))
                    .forEach(aggregator);
