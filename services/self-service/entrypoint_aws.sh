...
/usr/bin/keytool -importkeystore -srckeystore dlab.p12 -srcstoretype PKCS12 -alias dlab -destkeystore /root/keys/ssn.keystore.jks -deststorepass "${SSN_KEYSTORE_PASSWORD}" -srcstorepass "${SSN_KEYSTORE_PASSWORD}"
