...
mkdir -p /root/keys
sudo keytool -genkeypair -alias dlab -keyalg RSA -validity 730 -storepass password \
  -keypass password -keystore /root/keys/ssn.keystore.jks \
  -keysize 2048 -dname "CN=localhost"
sudo keytool -exportcert -alias dlab -storepass password -file /root/keys/ssn.crt \
