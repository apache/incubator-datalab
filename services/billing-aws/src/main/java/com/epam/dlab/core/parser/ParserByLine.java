...
public abstract class ParserByLine extends ParserBase {
	...
	public static final String ENTRY_NAME_STRING = "\nEntry name: ";
	public static final String SOURCE_LINE_STRING = "\nSource line[";
				...
				do {
					while ((line = getNextRow()) != null) {
						...
						try {
							...
							throw new ParseException(e.getLocalizedMessage() + ENTRY_NAME_STRING + getCurrentStatistics().getEntryName() +
									SOURCE_LINE_STRING +
									getCurrentStatistics().getRowReaded() + "]: " + line, e);
						} catch (Exception e) {
							throw new ParseException("Cannot evaluate condition " + getWhereCondition() + ". " +
									e.getLocalizedMessage() + ENTRY_NAME_STRING + getCurrentStatistics().getEntryName()
									+ SOURCE_LINE_STRING + getCurrentStatistics().getRowReaded() + "]: " + line, e);
						}

						reportLine = getReportLine(line, row);
	...
	private ReportLine getReportLine(String line, List<String> row) throws ParseException {
		ReportLine reportLine;
		try {
