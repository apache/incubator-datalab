...
public class DlabResourceTypeDAO implements MongoConstants {
    ...
    private void setResourceList() throws InitializationException {
    	...
    	for (Document exp : docs) {
    		...
    		resourceList.append(exploratoryId, exploratoryName, DlabResourceType.EXPLORATORY, username, exploratoryName);
			...
			for (Document comp : compList) {
				...
				resourceList.append(computationalId, computationalName, DlabResourceType.COMPUTATIONAL, username, exploratoryName);
			}
    	}
	...
	public Document transform(ReportLine row) throws ParseException {
		...
		if (resourceId == null || !resourceId.startsWith(serviceBaseNameId)) {
			throw new ParseException("DlabId is not match: expected start with " + serviceBaseNameId + ", actual " + resourceId);
		}
	...
	class BillingComparator implements Comparator<Document> {
		...
		public int compare(Document d1, Document d2) {
			...
			if (result == 0 ) {
				result = StringUtils.compare(d1.getString(ReportLine.FIELD_PRODUCT), d2.getString(ReportLine.FIELD_PRODUCT));
				if (result == 0 ) {
					return StringUtils.compare(d1.getString(ReportLine.FIELD_RESOURCE_TYPE), d2.getString(ReportLine.FIELD_RESOURCE_TYPE));
				}
			}
			return result;
		}
    ...
    private void updateExploratoryCost(String user, String exploratoryName) {
    	...
    	List<? extends Bson> pipeline = Arrays.asList(
    			...
    			sort(new Document(FIELD_ID + "." + FIELD_DLAB_RESOURCE_ID, 1).append(FIELD_ID + "." + ReportLine.FIELD_PRODUCT, 1))
    		);
    ...
    private void updateEdgeCost(String user) {
    	...
    	for(Document d : docs) {
    		Document id = (Document) d.get(FIELD_ID);
    		Bson values = Updates.combine(
					Updates.set(ReportLine.FIELD_COST, BillingCalculationUtils.round(d.getDouble(ReportLine.FIELD_COST), 2)),
